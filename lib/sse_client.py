# -*- coding: utf-8 -*-
"""Main module.
from : github/rtfol/aio-http-client
"""
import asyncio
import logging
import json
...
import attr
from aiohttp import hdrs, ClientSession, ClientConnectionError, ClientTimeout
...
class EventSource:
    ...
    async def connect(self, retry=0):
        ...
        _LOGGER.debug('connecting{} to {}'.format(headers, self._url))
        try:
...
async def tstHue(ipadr,appkey,ssl=True):
	evtypes= {'light':['dimming.brightness','on.on','color.xy.x','color.xy.y','color_temperature.mirek','owner.rtype'],
	 'grouped_lights':['on.on','dimming.brightness'],
	 'temperature':['temperature.temperature'],
	 'motion':['motion.motion'],
	 'light_level':['light.light_level'],
	 'device_power':['power_state.battery_level'],
	 'button':['button.last_event'],
	 'zigbee_connectivity':['status.'],
	 'bridge_home':['children'],
	 }
	UTCOFS = '+0200'
	...
	async with ClientSession(timeout=ClientTimeout(total=0)) as session:
		_LOGGER.info('session:{}'.format(session))
		async with EventSource(url, session=session, headers=headers, ssl=False) as event_source:
			try:
				async for event in event_source:
					...
					for dt in dat: # is a list
						...
						if 'Z' in tm:  # The timezone offset for UTC is 0 and would be indicated with the letter Z
							tm = tm.replace('Z',UTCOFS)  # tm[:-1]
						else:
							tm += UTCOFS
						#if '.' not in tm:
						#	tm += '.0'  # add milli sec for hue
						loct = datetime.strptime(tm, "%Y-%m-%dT%H:%M:%S%z") # aware utc
						val=None
						id=None
						for dati in  dt['data']:
							tp = dati['type']
								...
								if key and len(key)>0:
									for ky in key:
										...
										if ky[0] in dati:
											val = dati[ky[0]]
											break
							_LOGGER.info("loct:{}-[{}]=({}) ".format(loct,tp,val))
			except ConnectionError as ex:
				_LOGGER.error("error:{}".format(ex))

async def main(ipadr,user):
	await asyncio.gather(* [tstHue(ipadr,user)])
...
if __name__ == '__main__':	# just testing the API and gets userId if neccesary
	...
	_loop.run_until_complete(main(IP,appkey))
	#_loop.run_forever()
