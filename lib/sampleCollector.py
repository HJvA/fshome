	...
	def check_quantity(self,tstamp,quantity,val):
		...
		if self.qIsCounting(quantity): 
			...
			if (n>=self.minNr and abs(val-avg)>avg*self.minDevPerc/100) or n>=self.maxNr:
				logger.info('accepting avg n=%d avg=%g val=%s quantity=%s(%s) devPrc=%g tm=%s' % (n,avg,val,self.qname(quantity),quantity, abs(val-avg)/avg*100 if avg>0 else 0.0, prettydate(julianday(tstamp))))
