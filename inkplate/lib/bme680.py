...
class BME680(i2cPlus):
  def __init__(self, *args, refresh_rate=10, **kwargs):
    ...
    self._write_byte(_BME680_BME680_RES_HEAT_0, 0x73)
    self._write_byte(_BME680_BME680_GAS_WAIT_0, 0x65)
  ...
  def gas(self):
    self._perform_reading()
    if self._chip_variant == 0x01:   # taken from https://github.com/BoschSensortec/BME68x-Sensor-API
  ...
  def _perform_reading(self):
    if (time.ticks_diff(self._last_reading, time.ticks_ms()) * time.ticks_diff(0, 1)
      ...
      return
    self._write_byte(_BME680_REG_CONFIG, self._filter << 2)
    self._write_byte(_BME680_REG_CTRL_MEAS,
      (self._temp_oversample << 5)|(self._pressure_oversample << 2))
    self._write_byte(_BME680_REG_CTRL_HUM, self._humidity_oversample)
    if self._chip_variant == 0x01:
        self._write_byte(_BME680_REG_CTRL_GAS, _BME680_RUNGAS << 1)
    else:
        self._write_byte(_BME680_REG_CTRL_GAS, _BME680_RUNGAS)
    ctrl = self._read_byte(_BME680_REG_CTRL_MEAS)
    ctrl = (ctrl & 0xFC) | 0x01
    self._write_byte(_BME680_REG_CTRL_MEAS, ctrl)
    new_data = False
    while not new_data:
...
class BME680_I2C(Adafruit_BME680):
  def __init__(self, i2c, address=0x77, debug=False, *, refresh_rate=10):
    ...
    print("bme680 on:{}:{} with itv:{}".format(i2c, self._chip_variant, self._min_refresh_time))
  
  @classmethod
  def create(cls, ioSCL=22, ioSDA=21, addr=0x77):
     """ creates object with defaults for inkplate6 """
     i2c = I2C(0, scl=Pin(ioSCL), sda=Pin(ioSDA), freq=10000)
     if addr in i2c.scan():
         result = bytearray(1)
         chip_id = i2c.readfrom_mem_into(addr, _BME680_REG_CHIPID & 0xff, result)
         if chip_id != _BME680_CHIPID:
             return cls(i2c, addr)
     print("bme not found")
  def _read(self, register, length):
