...
class SCD30(i2cPlus):
    ...
    def __init__(self, *args, pause=1000, **kwargs):
        super().__init__(*args, **kwargs)  #i2c, addr, pause=1000):
        self.pause = pause
        print("scd30 on:{}:{} ".format(self.i2c, self._address))
        if self._address:
            print("scd v:{} titv:{} Tofs:{}".format(self.get_firmware_version(), self.get_measurement_interval(), self.get_temperature_offset()))

    def start_continous_measurement(self, ambient_pressure=0):
        bint = struct.pack('>H', ambient_pressure)
        crc = self.__crc(bint[0], bint[1])
        data = bint + bytes([crc])
        self.i2c.writeto_mem(self._address, self.START_CONT_MEASURE, data, addrsize=16)
    ...
    def set_measurement_interval(self, interval):
        ...
        crc = self.__crc(bint[0], bint[1])
        data = bint + bytes([crc])
        self.i2c.writeto_mem(self._address, self.SET_MEASURE_INTERVAL, data, addrsize=16)
    ...
    def set_automatic_recalibration(self, enable):
        ...
        crc = self.__crc(bint[0], bint[1])
        data = bint + bytes([crc])
        self.i2c.writeto_mem(self._address, self.SET_FRC, data, addrsize=16)
    ...
    def set_temperature_offset(self, offset):
        ...
        crc = self.__crc(bint[0], bint[1])
        data = bint + bytes([crc])
        self.i2c.writeto_mem(self._address, self.SET_TEMP_OFFSET, data, addrsize=16)
...
if __name__ == "__main__":	
	...
	#kl = klok.horloge()
	#print("scd30 ink display:{} x {} battery:{} T:{}°C".format( inktls.display.width(),inktls.display.height(),inktls.display.readBattery(), (esp32.raw_temperature()-32)/1.8))	  

	#i2c = I2C(1, scl=Pin(17), sda=Pin(18))  # see hwpdf
	#i2c = I2C(1, scl=Pin(22), sda=Pin(21))  # also hw pdf
	scd30 = SCD30.create(addr=0x61)  #(i2c, 0x61)

	while True:
		# Wait for sensor data to be ready to read (by default every 2 seconds)
		rdy = scd30.get_status_ready()
		utime.sleep_ms(1000)
		if rdy == 1:
			rec = scd30.read_measurement()
			print("{}->{}".format(rdy,rec))
		else:
			print("{}".format(rdy))