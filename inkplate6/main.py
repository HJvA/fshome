"""
micropython for inkplate 6 plus device showing fshome data
...
import inktls
from  inktls import scrWDTH,scrHGTH,showText,showNumber,showGraph,display
#import klok
...
qUNTS ={	231  : (20,70,  12,'T buro','°C',  1),
			...
			qkTtuin:(20,70, 12,'Ttuin', '°C',  1),
			qkTentr:(20,220, 12,'Tentr', '°C',  1),
			574  : (20,220, 12,'T terr','°C',  1),
			qkCO2: (scrWDTH-380,70, 12,'CO2',  'ppm',1),   # 403
...
qkSHOW = (qkTentr,912,915,300)  # for dbfetched FLDS
grKEY  = qkCO2   # graph
...
def showRest(qk, qval):
	...
	if qk in qUNTS:
		...
		lsize=max(size -8, 4)  # label size 
		lhght=int(max(height//2+6,40))
		#size = next(sz for sz,tp in qFLDS.items() if sz<=size)
		if DEBUG:
			print("show lbl:{}={} x:{},y:{},sz:{}".format(qk,lbl,x,y,size))
		wd,ht = showText(lbl, x=x,y=y-lhght, size=lsize)	#label
		if display:
			display.drawRoundRect(x, y-lhght, wd+10, height+lhght, 6, display.BLACK)
	else:
...
def fsRestGet(ipadr,port=8080, qkeys=[401,qkTentr], ssl=False, timeout=2, path='/lastval'):
...
def setTime():
	...
	if tm:
		...
		dst = tm['dst']
		print("dt:{} utcofs:{} tz:{} dst:{}".format(dt,utcofs,tz,dst))
	jd = julianday()
...
if __name__ == "__main__":	
	...
	inktls.display.drawImageFile(100, 350, "img/wi-cloud.bmp", invert=True)
	inktls.display.drawImageFile(201, 350, "img/wi-rain.bmp", invert=True)
	inktls.display.drawImageFile(302, 350, "img/wi-hail.bmp", invert=True)
	inktls.display.drawImageFile(403, 350, "img/wi-snow.bmp", invert=True)
	inktls.display.drawImageFile(504, 350, "img/wi-fog.bmp", invert=True)
	inktls.display.drawImageFile(605, 350, "img/wi-day-sunny.bmp", invert=True)
	inktls.display.drawImageFile(706, 350, "img/wi-humidity.bmp", invert=True)
	inktls.display.drawImageFile(807, 350, "img/wi-sleet.bmp", invert=True)
	...
	try:
		while True:
			...
			if scd is not None and scd.get_status_ready() == 1:
				...
				rst =fsRestGet(IP,qkeys=[qkCO2])
				if rst:
					showRest(qkCO2, rst[qkCO2])
			if bme:
				#time.sleep_ms(200)
		...
		print("main error:{} ex:{}".format( type(ex).__name__, ex))
